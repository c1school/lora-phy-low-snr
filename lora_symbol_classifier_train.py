...
# ============================================================
# LoRa Symbol Classifier 학습 스크립트
# ------------------------------------------------------------
# 이 파일이 수행하는 작업:
# 1) npz 데이터셋(train/val/test) 로드
# 2) noisy signal -> symbol index 를 직접 분류하는 1D CNN 학습
# 3) 학습 곡선(loss, accuracy) 저장
...
class TrainConfig:
    ...
    result_dir: str = os.path.join("results", "symbol_classifier_sf9")
...
class LoRaClassificationDataset(Dataset):
...
# ============================================================
# 모델: 1D CNN Symbol Classifier
# ------------------------------------------------------------
# 입력: noisy signal (B, 2, N)
# 출력: symbol logits (B, num_classes)
# ============================================================


class SymbolClassifier1D(nn.Module):
    def __init__(self, num_classes: int, base_channels: int = 32) -> None:
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm1d(base_channels * 4),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),  # N -> N/16
        )
...
def train_one_epoch(
    ...
    avg_loss = loss_sum / max(1, total_count)
    avg_acc = correct_sum / max(1, total_count)
...
def evaluate_ser_by_snr(
    model: nn.Module,
    test_dataset: LoRaClassificationDataset,
    params: LoRaParams,
    device: torch.device,
) -> Dict[str, List[float]]:
    """
    ...
    proposed:
      noisy -> CNN classifier -> pred symbol
    """
    model.eval()
...
def plot_loss_curve(
    ...
    plt.title("Symbol Classifier Loss Curve")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def plot_accuracy_curve(
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Symbol Classifier Accuracy Curve")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(save_path, dpi=200, bbox_inches="tight")
...
def plot_ser_compare(ser_result: Dict[str, List[float]], save_path: str) -> None:
    ...
    plt.semilogy(ser_result["snr_db"], ser_result["proposed_ser"], marker="o", label="Proposed (Symbol Classifier)")
    plt.xlabel("SNR (dB)")
    plt.ylabel("SER")
    plt.title("Baseline vs Symbol Classifier SER on Test Set")
...
def main() -> None:
    ...
    train_dataset = LoRaClassificationDataset(train_path)
    val_dataset = LoRaClassificationDataset(val_path)
    test_dataset = LoRaClassificationDataset(test_path)
    ...
    best_val_acc = -1.0
    best_model_path = os.path.join(cfg.result_dir, "best_symbol_classifier.pt")
