...
# 얘가 하는 일은 신경망 기반 denoising이 ultra-low SNR에서 LoRa 복조 성능을 개선하는가?”를 바로 실험하는거임.
# ============================================================
# - Baseline
#   noisy signal
#   dechirp + FFT
...
# ============================================================
# LoRa Denoising Autoencoder Training Script
# ------------------------------------------------------------
# 이 파일이 수행하는 작업:
# 1) npz 데이터셋(train/val/test) 로드
# 2) 1D CNN 기반 denoising autoencoder 학습
# 3) 학습 곡선(loss) 저장
...
class TrainConfig:
    ...
    result_dir: str = os.path.join("results", "denoiser_sf9")
...
# ============================================================
# 모델: 1D CNN Denoising Autoencoder
# ============================================================
...
def evaluate_ser_by_snr(
    ...
    proposed:
      noisy -> DNN denoiser -> dechirp + FFT -> pred symbol
    """
    model.eval()
...
def plot_ser_compare(ser_result: Dict[str, List[float]], save_path: str) -> None:
    ...
    plt.semilogy(ser_result["snr_db"], ser_result["proposed_ser"], marker="o", label="Proposed (Denoiser)")
    plt.xlabel("SNR (dB)")
    plt.ylabel("SER")
    plt.title("Baseline vs Proposed SER on Test Set")
...
def main() -> None:
    ...
    # 2) 모델 / 손실 / 옵티마이저
    # ------------------------
    model = DenoisingAutoencoder1D(hidden_channels=cfg.hidden_channels).to(device)
    ...
    best_model_path = os.path.join(cfg.result_dir, "best_denoiser.pt")
